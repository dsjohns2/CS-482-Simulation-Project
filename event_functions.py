...
def node_rider_request(sys, args):
	...
	new_event_time = sys.cur_time
	new_event = event.event("Available Driver", new_event_time, node_available_driver, args={'rider_id': rider_id, 'rider_location': rider_location})
	eventlist_tuple = (new_event_time, new_event)
	heapq.heappush(sys.eventlist, eventlist_tuple)

    print("")

def node_available_driver(sys, rider_id, rider_location):
...
def node_driver_response(sys, args):
    ...
    rider_id = args['rider_id']
    sys.driver_current_locations[driver_id] = sys.rider_start_location[rider_id]
