...
def node_rider_request(sys, args):
    ...
    new_event_time = sys.cur_time
    new_event = event.event("Update Driver Location", new_event_time, node_update_driver_locations, args={
                            'rider_id': rider_id, 'rider_location': rider_location}, priority=1)
    eventlist_tuple = (new_event_time, new_event)
    heapq.heappush(sys.eventlist, eventlist_tuple)
...
def node_update_driver_locations(sys, args):
    ...
    for driver_id in range(0, len(sys.drivers_free_at_last_update)):
        if(sys.drivers_free_at_last_update[driver_id]):
            ...
            if(abs(x_diff) > dist_diff):
                ...
                sys.driver_current_locations[driver_id] = (
                    cur_x + dist_diff * np.sign(x_diff), cur_y)
            else:
                # Some movement in y direction
                rem_dist_to_move = dist_diff - abs(x_diff)
                y_diff = checkpoint[1] - \
                    sys.driver_current_locations[driver_id][1]
                if(abs(y_diff) > rem_dist_to_move):
                    # Move some toward checkpoint
                    sys.driver_current_locations[driver_id] = (
                        cur_x + x_diff, cur_y + rem_dist_to_move * np.sign(y_diff))
                else:
                    # Move all the way to checkpoint
                    sys.driver_current_locations[driver_id] = (
                        cur_x + x_diff, cur_y + y_diff)
    ...
    # Schedule events
    new_event_time = sys.cur_time
    new_event = event.event("Available Driver", new_event_time,
                            node_available_driver, args=args, priority=2)
    eventlist_tuple = (new_event_time, new_event)
    heapq.heappush(sys.eventlist, eventlist_tuple)
...
def node_available_driver(sys, args):
    ...
    if free_driver:
        ...
        new_event = event.event("Driver Response", new_event_time, node_driver_response, args={
                                'rider_id': rider_id, 'driver_id': min_driver_id}, priority=3)
        eventlist_tuple = (new_event_time, new_event)
        heapq.heappush(sys.eventlist, eventlist_tuple)
